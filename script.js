const API_URL =
  "https://script.google.com/macros/s/AKfycbzRTocdnvtQelDHxZhOHzOyTM6sK7qB7QDL61m28qqZoZ1PkAqdSBvWi4azEPpOCJY/exec";
...
function mulaiUjian() {
  ...
  if (!nama || !sekolah || !gender || !daerah || !tinggi || !berat) {
  ...
  localStorage.setItem("kabkota", daerah);
...
function mulaiTimer() {
  timer = setInterval(() => {
    ...
    document.getElementById("time").textContent = `${menit}:${detik}`;
...
function semuaTerjawab(){
  ...
  for(let i=0;i<soal.length;i++){
    ...
    if(jawaban[i] === undefined) return i;
...
function kirim(){
  ...
  document.getElementById("nextBtn").textContent = "Mengirim...";
